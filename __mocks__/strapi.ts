...
const plugins = (strapi: IStrapi): StringMap<StrapiPlugin> => ({
  navigation: {
    ...
    get contentTypes() {
      return require('../server/content-types');
    },
    contentType: (key: string) =>
      preparePluginContentType(
        require('../server/content-types').default[key].schema,
        'navigation'
      ),
    config: (key: string) =>
      ({
        ...
        additionalFields: [
          { name: 'string_test_field', label: 'Test field', type: 'string', enabled: true },
        ],
      })[key],
    get controllers() {
      return {};
...
const strapiFactory = (
  plugins: (strapi: IStrapi) => StringMap<StrapiPlugin>,
  contentTypes: StringMap<StrapiContentType<ToBeFixed>>
) => ({
  get plugins() {
    return plugins(strapi);
  },
