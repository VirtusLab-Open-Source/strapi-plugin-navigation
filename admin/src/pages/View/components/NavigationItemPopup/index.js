...
const NavigationItemPopUp = ({
  ...
  const appendLabelPublicationStatus = (
    ...
    const appendix = isRelationPublished({
      ...
      ? ""
      : `[${getMessage(
          "notification.navigation.item.relation.status.draft"
        )}] `.toUpperCase();
    return `${appendix}${label}`;
  };

  const relatedTypeItem = find(
    contentTypes,
    (item) => item.uid === relatedType
  );
  const prepareFormData = (data) => {
    ...
    return {
      ...
      related:
        related && isRelationCorrect(data)
          ? {
              value: related,
          ...
          : undefined,
      relatedType:
        relatedType && isRelationCorrect(data)
          ? {
              value: relatedType,
              label: appendLabelPublicationStatus(
                relatedTypeItem.label || relatedTypeItem.name,
                relatedTypeItem,
                true
              ),
            }
